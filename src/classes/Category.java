...
public class Category
{
	...
	/** @description will search through the whole List of all Patterns and add all words to the category that fulfill the conditions to be part of
	 * 				 the category. These conditions are: 
	 * 				(1) At least unidirectional connected to all words from the clique and 
	 * 				(2) bidirectional connected to at least one word of the clique
	 *  
	 * @param finalPatternList is the List of all Pattern that were collected doing the M1, M2 and M3 measurements
	 * @param allWords contains all the words from the finalPatternList as Strings
	 * 
	 * TODO Die boolean fulfillsConditions braucht man wegen dem Break eigentlich nicht. Tests schreiben, ob es auch ohne geht.
	 */
	...
	/** @description Tests if the given word is unidirectional connected to all words in the clique
	 */
	private boolean uniDirectionalToAll(String word, LinkedList<Pattern> finalPatternList)
	{
		...
		if( !(hasArc(word, cliqueFirst, finalPatternList) || hasArc(cliqueFirst, word, finalPatternList)) ) return false;
		
		// ...and any connection to the second word of the clique.
		if( !(hasArc(word, cliqueSecond, finalPatternList) || hasArc(cliqueSecond, word, finalPatternList)) ) return false;
		
		return true;
	}

	
	/** @description Tests if the given word is bidirectional connected to at least one word in the clique
	 */
	private boolean biDirectionalToOne(String word, LinkedList<Pattern> finalPatternList)
	{
		...
		if( hasArc(word, cliqueFirst, finalPatternList) && hasArc(cliqueFirst, word, finalPatternList) ) bidirectional = true;
		
		// ... or the second word of the clique.
		if( hasArc(word, cliqueSecond, finalPatternList) && hasArc(cliqueSecond, word, finalPatternList) ) bidirectional = true;
	...
	/** @description Tests if the offered Category contains >= 50 % of this category. If yes, it will merge both categories and return true, else returns false. 
	 */
	public boolean offerCategory(Pattern offeredCategory)
	{
